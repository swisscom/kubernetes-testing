...
if Config.longhorn_enabled
  describe 'a kubernetes deployment with a persistent volume', :deployment => true do
    before(:all) do
      @kubectl = KUBECTL.new()
      @name = Config.random_names ? random_name('deployment') : 'test-deployment'
    end
    ...
    context 'when deployed' do
      ...
      it "has a bound persistent volume" do
        ...
        wait_until(240,15) {
          pvcs = @kubectl.get_objects("pvc")['items']
          expect(pvcs).to_not be_nil
          expect(pvcs.count).to be >= 1

          pvcs.map! { |pvc| "#{pvc['metadata']['name']}/#{pvc['status']['phase']}" }
          expect(pvcs).to include("#{@name}/Bound")
        }
      end
        ...
        context 'with an Ingress' do
          ...
          if Config.lets_encrypt_enabled
            context 'with a valid certificate' do
              ...
              it "can be https queried via domain [#{Config.domain}]" do
                wait_until(30,5) {
                  response = https_get("https://#{@name}.#{Config.domain}/hi")
                  expect(response).to_not be_nil
                  expect(response.code).to eq(200)
              ...
              it "can validate the app config via domain [#{Config.domain}]" do
                wait_until(30,5) {
            ...
            it "can be http queried via domain [#{Config.domain}]" do
              wait_until(30,5) {
                response = http_get("http://#{@name}.#{Config.domain}/hi")
                expect(response).to_not be_nil
                expect(response.code).to eq(200)
                expect(response.headers[:content_type]).to include('text/plain')
                expect(response.body).to eq('Hello')
